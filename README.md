...
从视频文件中提取帧的工具，支持 **Web 可视化界面** 和 **命令行** 两种使用方式。
...
```bash
# 1. 克隆项目
git clone https://github.com/Zhoumornnig/frame-extraction-tool.git
cd frame-extraction-tool

# 2. 安装依赖
pip install -r requirements.txt

# 3. 启动（二选一）
python app.py                    # Web 可视化界面（推荐）
python -m frame_extractor ...    # 命令行模式
```
...
- **上传视频** — 拖拽或选择任意视频文件
- **选择模式** — 4 种提取模式一键切换
- **调整参数** — 滑块实时调节间隔、阈值、质量等
- **结果预览** — 提取的帧以画廊形式展示
- **打开文件夹** — 一键打开输出目录

> 💡 如需生成公网链接分享给他人使用：将 `app.py` 末尾的 `demo.launch(...)` 中添加 `share=True`
...
with FrameExtractor("video.mp4", output_dir="frames", fmt="jpg") as e:
    ...
    # 关键帧提取
    files = e.extract_keyframes(threshold=0.5)
```
